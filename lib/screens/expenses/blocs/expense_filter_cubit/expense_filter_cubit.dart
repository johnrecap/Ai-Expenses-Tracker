...
class ExpenseFilterCubit extends Cubit<ExpenseFilterState> {
  ...
  void replaceExpenses(List<Expense> expenses, {bool? hasMoreLoadedScope}) {
    _allExpenses = List.unmodifiable(_orderedUnique(expenses));
    _hasMoreLoadedScope = hasMoreLoadedScope ?? _hasMoreLoadedScope;
    _apply(state.filter);
  }

  void mergeRecentExpenses(List<Expense> expenses, {bool? hasMoreLoadedScope}) {
    _allExpenses = List.unmodifiable(
      _orderedUnique([...expenses, ..._allExpenses]),
    );
    _hasMoreLoadedScope = hasMoreLoadedScope ?? _hasMoreLoadedScope;
    _apply(state.filter);
  }

  void appendExpenses(List<Expense> expenses, {bool? hasMoreLoadedScope}) {
    _allExpenses = List.unmodifiable(
      _orderedUnique([..._allExpenses, ...expenses]),
    );
    _hasMoreLoadedScope = hasMoreLoadedScope ?? _hasMoreLoadedScope;
    _apply(state.filter);
  }
