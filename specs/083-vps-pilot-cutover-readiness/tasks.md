...
- [ ] T040 [Owner] Run staging backfill against the selected account and send sanitized output. Why: credentials and server access are owner-controlled. Expected: import completes or clear errors appear. Risk: hidden migration failure. Status: first run exposed that the current backfill script still imports into an in-memory target; Codex must replace it with PostgreSQL writes before this can be marked complete.
